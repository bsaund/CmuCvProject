...
int singleDepthMap(Mat img1, Mat img2, Mat img1_colored, Mat img2_colored, 
	...
	Mat dispIntL, dispIntR, dispIntFilt;
	sgbm->compute(img1, img2, dispIntL);
	dispIntL.convertTo(dispL, CV_32F);
	dispL /= 16;  //sgbm returns disp as a 4-fractional-bit short
	
	Ptr<StereoMatcher> right_matcher = createRightMatcher(sgbm);

	// right_matcher->compute(img2, img1, dispIntR);

	// dispIntR.convertTo(dispR, CV_32F);
	// dispR /= 16;
	...
	// namedWindow("dispR", 1);
	// imshow("dispR", dispR/176);
	// namedWindow("dispL", 1);
	// imshow("dispL", dispL/176);
